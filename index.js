...
app.post("/carrito", async (req, res) => {
  try {
    ...
    const { id_producto, nombre_producto, cantidad_producto, precio_producto, imagen_producto } = req.body;

    // Verificar que los datos estén presentes
    if (!id_producto || !nombre_producto || !cantidad_producto || !precio_producto || !imagen_producto) {
      return res.status(400).send("Datos incompletos.");
    }

    // Calcular el total (precio por cantidad)
    const total = precio_producto * cantidad_producto;

    // Consulta SQL para insertar los datos en la tabla "ventaencaja"
    const query = `
      INSERT INTO ventaencaja (id_producto, nombre_producto, cantidad_producto, precio_producto, imagen_producto, total)
      VALUES ($1, $2, $3, $4, $5, $6)
    `;

    // Ejecutar la consulta con los datos del formulario
    await sql(query, [id_producto, nombre_producto, cantidad_producto, precio_producto, imagen_producto, total]);
    ...
    res.status(500).send("Ocurrió un error al procesar el carrito.");
  }
});